...
public class div extends JFrame {
	 ...
	 * Create the frame.
	 */
	public div(String username, String firstname) {
		uname = username;
		fname = firstname;
...
public void generateRandom(){
		...
		int n2 = randomClass.nextInt(10)+1;
		...
		int index = randomClass.nextInt(4);  // Picks random box from 4
		 ...
		 for(int i=0;i < 3; i++){
			 ...
			 double tst = (double)Result; // right answer in tst
			 double tmp = randomClass.nextInt((int)(tst+10))+1;  // Random answer between 0-Result+10
		 ...
		 for(int i=0; i<4; i++) { 
				AnswerButton[i].setText(String.valueOf((double)answers[i])); // .split("\\.")[0]
				}
		...
		for(i=0; i < 4; i++){
			...
			AnswerButton[i].addMouseListener(new MouseAdapter() {
				public void mouseClicked(MouseEvent e) {
					...
					answer = Double.parseDouble(AnswerButton[innerAnswerIndex].getText());
					if(answer == Result){
